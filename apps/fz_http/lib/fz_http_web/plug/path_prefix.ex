defmodule FzHttpWeb.Plug.PathPrefix do
  ...
  defp trim_prefix(
       ...
       do: %{conn | path_info: path_info, request_path: Enum.join([""] ++ request_path_info, "/")}

  defp trim_prefix(%Plug.Conn{path_info: path_info} = conn, _request_path_info, prefix),
    do: Phoenix.Controller.redirect(conn, to: "/" <> Path.join(prefix, path_info))
end
